...
// "Fixed ultra-fast sprint bug when crouching, fixed operator priority in isWalking check" || version 1.0.2
// "Added Footstep sounds and AI Noise radius system" || version 1.0.3
...
public class FirstPersonController : MonoBehaviour
{
    ...
    #region Footstep Sounds & AI Noise
    ...
    [Header("Noise Radius (For AI)")]
    public float walkNoiseRadius = 10f;
    ...
    // Public properties for AI to read state if needed
    public bool IsMoving => isWalking;
    public bool IsSprintingState => isSprinting;
    public bool IsCrouchedState => isCrouched;

    /// <summary>
    /// Returns the current noise radius based on player movement state.
    /// </summary>
    public float GetNoiseRadius()
    {
        if (!isWalking || !isGrounded) return 0f;
        if (isSprinting) return sprintNoiseRadius;
        if (isCrouched) return crouchNoiseRadius;
    ...
    private void Update()
    {
        ...
        if (footstepAudioSource != null && isGrounded && rb.linearVelocity.magnitude > 0.1f)
        {
            if (isWalking)
            {
                ...
                if (stepTimer <= 0f)
                {
                    AudioClip[] currentSounds = walkSounds;
                    float interval = walkStepInterval;

                    if (isSprinting && sprintSounds != null && sprintSounds.Length > 0)
                    {
                        currentSounds = sprintSounds;
                        interval = sprintStepInterval;
                    }
                    else if (isCrouched && crouchSounds != null && crouchSounds.Length > 0)
                    {
                        currentSounds = crouchSounds;
                    ...
                    if (currentSounds != null && currentSounds.Length > 0)
                    {
                        ...
                        footstepAudioSource.pitch = Random.Range(0.9f, 1.1f); // Slight pitch variation for realism
            ...
            {
                stepTimer = 0f; // Reset so it plays immediately when starting to move
            }
        }
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            // FIX: Added parentheses to fix operator priority bug
    ...
    // FIX: Separated into Crouch and StandUp to prevent double-applying speed reduction
    private void Crouch()
    {
        if (isCrouched) return; // Prevent double crouching
    ...
    {
        if (!isCrouched) return; // Prevent double standing up (This caused the ultra-fast sprint bug!)
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.Space();
        EditorGUILayout.LabelField("", GUI.skin.horizontalSlider);
        GUILayout.Label("Footstep Sounds & AI Noise", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Bold, fontSize = 13 }, GUILayout.ExpandWidth(true));
        EditorGUILayout.Space();

        fpc.footstepAudioSource = (AudioSource)EditorGUILayout.ObjectField(new GUIContent("Audio Source", "Audio source to play footstep sounds."), fpc.footstepAudioSource, typeof(AudioSource), true);
        
        SerializedProperty walkSoundsProp = SerFPC.FindProperty("walkSounds");
        EditorGUILayout.PropertyField(walkSoundsProp, true);
        
        SerializedProperty sprintSoundsProp = SerFPC.FindProperty("sprintSounds");
        EditorGUILayout.PropertyField(sprintSoundsProp, true);
        
        SerializedProperty crouchSoundsProp = SerFPC.FindProperty("crouchSounds");
        EditorGUILayout.PropertyField(crouchSoundsProp, true);

        EditorGUILayout.Space();
        GUILayout.Label("Step Intervals", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleLeft, fontStyle = FontStyle.Bold, fontSize = 12 }, GUILayout.ExpandWidth(true));
        fpc.walkStepInterval = EditorGUILayout.Slider("Walk Interval", fpc.walkStepInterval, 0.1f, 2f);
        fpc.sprintStepInterval = EditorGUILayout.Slider("Sprint Interval", fpc.sprintStepInterval, 0.1f, 2f);
        fpc.crouchStepInterval = EditorGUILayout.Slider("Crouch Interval", fpc.crouchStepInterval, 0.1f, 2f);

        EditorGUILayout.Space();
        GUILayout.Label("Noise Radius (For Monster AI)", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleLeft, fontStyle = FontStyle.Bold, fontSize = 12 }, GUILayout.ExpandWidth(true));
        fpc.walkNoiseRadius = EditorGUILayout.Slider("Walk Noise Radius", fpc.walkNoiseRadius, 0f, 50f);
        fpc.sprintNoiseRadius = EditorGUILayout.Slider("Sprint Noise Radius", fpc.sprintNoiseRadius, 0f, 50f);
        fpc.crouchNoiseRadius = EditorGUILayout.Slider("Crouch Noise Radius", fpc.crouchNoiseRadius, 0f, 50f);
